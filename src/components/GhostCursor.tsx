...
const GhostCursor = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      (composer as any).setPixelRatio?.(pixelRatio);
    ...
    const animate = () => {
      ...
      if ((filmPassRef.current as any)?.uniforms?.iTime) {
        (filmPassRef.current as any).uniforms.iTime.value = t;
      }
  ...
  useEffect(() => {
    if ((filmPassRef.current as any)?.uniforms?.intensity) {
      (filmPassRef.current as any).uniforms.intensity.value = grainIntensity;
    }
  }, [grainIntensity]);
